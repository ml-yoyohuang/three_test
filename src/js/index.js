...
const app = new Vue({
  ...
  methods: {
    init() {
      ...
      this.geometry = new THREE.CubeGeometry(0.9, 0.9, 0.9); // Size
      ...
      this.materiall = new THREE.MeshPhongMaterial({ map: texture }); // light enable

      for (let index = 0; index < 27; index++) {
        this.coords[index] = [Math.floor(index / 9), Math.floor((index % 9) / 3), index % 3];
        this.cubes[index] = new THREE.Mesh(this.geometry, this.materiall);
        this.cubes[index].position.x = this.coords[index][0] - 1;
        this.cubes[index].position.y = this.coords[index][1] - 1;
        this.cubes[index].position.z = this.coords[index][2] - 1;
    ...
    render() {
      ...
      if (this.camera.position.y > 10) { this.camera.position.y = 10; } else if (this.camera.position.y < -10) { this.camera.position.y = -10; }
      this.cameradeg += mouse.x * 0.03; // rotate camera
      this.cameradeg = this.cameradeg % (Math.PI * 2);
      this.camera.position.x = Math.sin(this.cameradeg) * 10;
      this.camera.position.z = Math.cos(this.cameradeg) * 10;
