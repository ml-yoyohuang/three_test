...
const app = new Vue({
  ...
  methods: {
    ...
    createCubes() {
      this.geometry = new THREE.CylinderGeometry(2, 2, 700, 3); // Size
      for (let index = 0; index < 50; index++) {
        this.coords[index] = [Math.random() * 700, Math.random() * 700, Math.random() * 700];
        this.cubes[index] = new THREE.Mesh(this.geometry, this.materialc);
        this.cubes[index].position.x = this.coords[index][0] - 350;
        this.cubes[index].position.y = 0;
        this.cubes[index].position.z = this.coords[index][2] - 350;
    ...
    render() {
      ...
      if (this.camera.position.y > 500) { this.camera.position.y = 500; } else if (this.camera.position.y < -500) { this.camera.position.y = -500; }
      this.cameradeg += mouse.x * 0.005; // rotate camera
      this.cameradeg = this.cameradeg % (Math.PI * 2);
      this.camera.position.x = Math.sin(this.cameradeg) * 500;
      this.camera.position.z = Math.cos(this.cameradeg) * 500;
      this.camera.lookAt(0, 0, 0);
